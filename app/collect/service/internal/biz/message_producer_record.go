...
type MessageProducerRecordModel struct {
...
type MessageProducerRecordRepo interface {
	DeleteBySendTime(string) (int64, error)
}

type MessageProducerRecordUsecase struct {
	repo MessageProducerRecordRepo
	log  *log.Helper
}

func NewMessageProducerRecordUsecase(repo MessageProducerRecordRepo, logger log.Logger) *MessageProducerRecordUsecase {
	return &MessageProducerRecordUsecase{repo: repo, log: log.NewHelper(logger)}
}
